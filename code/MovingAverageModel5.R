...
#Editor: Michael Lane
# ptm <- proc.time(); myAvg <- instanceRun(); B <- proc.time() - ptm; print(B)
#To intallize hit source on save
### --- THe code
#testDataDF <- readRDS("data/testData.rds")
# 
#

instanceRun <- function(num=3, method="runAvgP")
{
  testFile <- paste0("data/", "testData", num, ".rds")
  outFile <- paste0("data/submission/average/testData", num, ".csv")
  runAvgP(readRDS(testFile), readRDS("data/newItempredictions.rds"), outFile)
}

combineOutput <- function(outpath="data/submission/average", submitFile="data/submission/average/avg.csv")
{
  require(data.table)
  ...
  for(file in files)
  {
    ...
    {
      write.table(dataDF, file=submitFile, append=TRUE, sep=",", col.names=FALSE, row.names=FALSE, quote=FALSE)
    }
    else
    {
      write.table(dataDF, file=submitFile, append=FALSE, sep=",", col.names=FALSE, row.names=FALSE, quote=FALSE)
    }
    #fix the names
    myAvg <- fread("data/submission/average/avg.csv")
    names(myAvg) <- c("id","unit_sales")
    #submission[nm] <- lapply(nm, function(x) submission[[x]][match(submission$id, myAvg$id)])
    submission <- fread("data/oldoutput1.csv")
    newsubmission <- data.table(merge(myAvg,submission,by = "id", all.y = TRUE))
    colnames(newsubmission)[2] <- "unit_sales"
    newsubmission$unit_sales.y <- NULL
    newsubmission[is.na(newsubmission)] <-0 
    fwrite(newsubmission,"data/submission/avgfinal.csv",sep = ",")
  } 
}

mainRun <- function(numSplits=4, outpath="data/submssion/average/", submitFile="data/submission/average/avg.csv")
{
...
runAvgP <- function(testDataDF = NULL, newItemsDF = NULL, outfile=NULL)
{
  require(foreach)
  require(doParallel)
  require(readr)
  ...
  #1:100000 .verbose = TRUE
  out.data <- foreach(x = 1:nrow(testDataDF), .combine = rbind, .export=c("computeAvgP","newItemsDF","read_csv")) %dopar%
  {
  ...
  write.table(out.data, file=outfile, sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE)
}
...
computeAvgP <- function(rowID, date, itemNbr, storeNbr, newItemsDF)
{
  ...
  {
    model.train <- read_csv(filename[1])
